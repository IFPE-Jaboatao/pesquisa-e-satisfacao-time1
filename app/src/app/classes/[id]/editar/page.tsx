...
export default function EditarClassePage() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3001/classes/${params.id}`)
      .then((r) => r.json())
      .then((cl) => setForm({ nome: cl.nome, codigo: cl.codigo, cursoId: cl.cursoId, semestre: cl.semestre, ano: cl.ano }))
      .catch(() => {});
    fetch("http://localhost:3001/cursos").then((r) => r.json()).then(setCursos).catch(() => {});
  }, [params.id]);

  const handleSubmit = async () => {
    await fetch(`http://localhost:3001/classes/${params.id}`, {
      method: "PUT",
    ...
    <div className="page-wrapper">
      ...
      <div className="page-content">
        <div className="form-card">
          <h2 className="form-title">Atualizar de Classes</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div className="form-group">
              ...
              <input className="form-control" type="number" value={form.ano}
                onChange={(e) => setForm({ ...form, ano: e.target.value })} />
            </div>
          </div>
          <button className="btn btn-primary" onClick={handleSubmit}>Modificar Classe</button>
        </div>
      </div>
    </div>
  );
