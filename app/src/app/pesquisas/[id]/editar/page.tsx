...
export default function EditarPesquisaPage() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3001/pesquisas/${params.id}`)
      .then((r) => r.json())
      .then((p) => {
        setForm({ titulo: p.titulo, descricao: p.descricao, campusId: p.campusId,
          cursoId: p.cursoId, servicoId: p.servicoId,
          dataInicial: p.dataInicial, dataFinal: p.dataFinal, anonima: p.anonima });
        setQuestoes(p.questoes ?? []);
      }).catch(() => {});
  ...
  const handleSubmit = async () => {
    await fetch(`http://localhost:3001/pesquisas/${params.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, questoes }),
    });
    router.push(`/pesquisas/${params.id}`);
  };
    ...
    <div className="page-wrapper">
      ...
      <div className="page-content">
        <div className="form-card" style={{ maxWidth: 600 }}>
          <h2 className="form-title">Atualização de Pesquisa</h2>
          ...
          {questoes.map((q, i) => (
            <div key={i} className="card" style={{ padding: 16, marginBottom: 12 }}>
              <div className="form-group">
                ...
                <input className="form-control" value={q.pergunta}
                  onChange={(e) => updateQuestao(i, "pergunta", e.target.value)} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div className="form-group">
          ...
          <button className="btn btn-primary" onClick={handleSubmit}>Atualizar Pesquisa</button>
        </div>
      </div>
    </div>
  );
